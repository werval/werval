 ...
 * GuiceGlobal.
 */
public class GuiceGlobal
    extends Global
{
    @Override
    public <T> T getFilterInstance( Application application, Class<T> filterType )
    {
        return application.plugin( Injector.class ).getInstance( filterType );
    }

    @Override
    public <T> T getControllerInstance( Application application, Class<T> controllerType )
    {
        return application.plugin( Injector.class ).getInstance( controllerType );
    }
}
